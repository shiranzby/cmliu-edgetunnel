...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		优选订阅生成: {
			...
			CFIPDataURL: env.CFIP_DATA_URL || 'https://raw.githubusercontent.com/shiranzby/cmliu-edgetunnel/main/data/cfip.json',
			本地IP库: {
	...
	if (!config_JSON.优选订阅生成.CFIPDataURL) config_JSON.优选订阅生成.CFIPDataURL = env.CFIP_DATA_URL || 'https://raw.githubusercontent.com/shiranzby/cmliu-edgetunnel/main/data/cfip.json';
